...
from artexhibitionlib import enquiry_response
...
# def s3_image_upload(file_obj, object_name=None):
#     bucket_name = "art-exhibotion-s3"
...
# def send_email_sns(subject, message, art_product, email, phone):
#     SNS_TOPIC_ARN = "arn:aws:sns:eu-west-2:250738637992:art-exhibition-cpp-sns"
...
def ENQUIRY_DETAILS(request):
    if request.method == "POST":
        try:
            ...
            msg = enquiry_response(art_product.title, request.POST['email'],request.POST['mobnum'],request.POST['message'])
            # # SNS call
            # if send_email_sns("Enquiry",request.POST['message'], art_product.title, request.POST['email'],request.POST['mobnum']):
...
#     """
#     API_URL = "https://p4wbnyopmg.execute-api.eu-west-2.amazonaws.com/stage1/art-exhibition-lambda"
...
def ADD_ARTIST(request):
    if request.method == "POST":
        try:
            ...
            # Send an email notification via API Gateway
            ...
            email_response = send_email_api(email_subject, email_message, "projectmail2223@gmail.com")
    ...
    if request.method == "POST":
        try:
            ...
            artist_obj = Artist(
                name=request.POST['name'],
                mobnum=request.POST['mobnum'],
                email=request.POST['email'],  # Taking email from form input
                edudetails=request.POST['edudetails'],
                awarddetails=request.POST['awarddetails'],
                images=request.FILES.get('images')
            )
...
from artapp.models import Enquiry
...
def UPDATE_ENQUIRY_REMARK(request):
    ...
    print(f"Enquiry ID: {enq_id}")
    print(f"Remark Text: {remark_text}")
    ...
    try:
        ...
        enquiry.remark_date = timezone.now()
        enquiry.save()
        ...
        message = (
            f"Dear {enquiry.fullname},\n\n"
            f"Thank you for contacting us.\n\n"
            f"Response to your enquiry:\n{remark_text}\n\n"
            "If you need further assistance, feel free to reach us again.\n\n"
            "Regards,\nArt Exhibition Team"
        )
        ...
        messages.success(request, "Enquiry updated and email sent successfully")
...
# code for the SMTP email response to enquiry which will be given by admin to the customer
def smtp_email_response_to_enquiry(art_product, email, phone, message):
    subject = f"Response to your enquiry about '{art_product}'"
    body = f"""Dear Customer,
Thank you for your enquiry about our art product '{art_product}'. We appreciate your interest.
Here is our response to your enquiry:
{message}
If you have any further questions, please feel free to contact us at {email} or call us at {phone}.
Best regards,
Art Store Team"""
...
# def VIEW_ENQUIRY_RESPOND(request, id):
#     try:
...
#             # Update enquiry
#             enquiry.response_text = response_text
#             enquiry.status = "Responded"
#             enquiry.save()

#             # Send Email to Customer via SMTP
#             subject = "Response to Your Art Exhibition Enquiry"
#             message = (
#                 f"Dear {enquiry.name},\n\n"
#                 f"Thank you for contacting us.\n\n"
#                 f"Your Enquiry: {enquiry.message}\n\n"
#                 f"Our Response: {response_text}\n\n"
#                 f"Regards,\nArt Exhibition Management Team"
#             )
...
#             messages.success(request, "Response sent to customer successfully.")
